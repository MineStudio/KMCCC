namespace KMCCC.Launcher
{
	...
	public static class LauncherCoreItemResolverExtensions
	{
		...
		{
			return String.Format(SystemTools.GetIsWindows() ? @"{0}\versions\{1}\" : "{0}/versions/{1}/", core.GameRootPath, versionId);
		}
		...
		{
			return String.Format(SystemTools.GetIsWindows() ? @"{0}\versions\{1}\{1}.jar" : "{0}/versions/{1}/{1}.jar", core.GameRootPath, versionId);
		}
